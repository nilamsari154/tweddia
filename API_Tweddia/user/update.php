...
if(isset($data->id)){
    ...
    if($get_stmt->rowCount() > 0){
        ...
        $post_handphone = isset($data->handphone) ? $data->handphone : $row['handphone'];
        $post_name = isset($data->name) ? $data->name : $row['name'];
        $post_password = isset($data->password) ? $data->password : $row['password'];
        
        $update_query = "UPDATE `user` SET email = :email, handphone = :handphone, password = :password, name = :name WHERE id = :id";
        
        $update_stmt = $conn->prepare($update_query);
        
        // DATA BINDING AND REMOVE SPECIAL CHARS AND REMOVE TAGS
        $update_stmt->bindValue(':email', htmlspecialchars(strip_tags($post_email)),PDO::PARAM_STR);
        $update_stmt->bindValue(':handphone', htmlspecialchars(strip_tags($post_handphone)),PDO::PARAM_STR);
